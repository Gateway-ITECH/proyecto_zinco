...
(function (Drupal) {
  ...
  Drupal.behaviors.zincoRetosList = {
    attach: function (context, settings) {
      once('zincoRetosList', '.zinco-retos-list', context).forEach(function (element) {
        // global variables
        const urlParams = new URLSearchParams(window.location.search);
        const searchInput = element.querySelector('#search-filter');

        // Toggle filter card visibility
        const filterButton = element.querySelector('#toggle_retos_filter');
        const filterCard = element.querySelector('#retos_filters');
